...
- 版本：`v0.27.0-auto-resolved-holdings`
- 阶段：当前持仓自动补全
- 业务能力：完整 ETF 投研链路、严格数据门禁、管理员鉴权、生产部署、自动数据库迁移、备份恢复、独立工作流 worker、外部 Caddy 接入和代码自动解析式持仓录入
...
- 代码自动解析式持仓录入：用户只填写代码、持仓数量和成本价；系统根据资产主表补全名称/类型，根据最新行情补全现价，并自动计算市值、成本、盈亏和权重。
