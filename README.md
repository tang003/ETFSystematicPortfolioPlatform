...
- 版本：`v0.65.12-docs-deploy-consistency`
- 阶段：Tushare-only 数据模式、后台任务中心、操作审计、多用户数据隔离、角色权限与部署文档一致性巡检
...
- 权限与审计：支持环境变量兜底管理员和数据库用户，角色包括管理员、研究员、观察者；变更类 API 会写入操作审计日志。
- 用户数据隔离：当前持仓、定投计划、报告和 AI 投研历史会绑定当前登录用户，ETF 池、行情、因子和目标组合仍作为共享研究数据。
