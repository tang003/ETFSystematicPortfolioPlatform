...
- 版本：`v0.29.1-etf-pool-route-fix`
- 阶段：ETF 池筛选器增强
- 业务能力：完整 ETF 投研链路、严格数据门禁、管理员鉴权、生产部署、自动数据库迁移、备份恢复、独立工作流 worker、外部 Caddy 接入、弹窗式持仓维护和 ETF 池筛选器
...
- ETF 池：管理和查询系统当前启用的 ETF 主数据，支持类别、区域、风险、跨境和启用状态筛选。
...
- 弹窗式持仓维护：新增、编辑和补仓都通过弹窗完成，用户只填写代码、持仓数量和成本价；系统根据资产主表补全名称/类型，根据最新行情补全现价，并自动计算市值、成本、盈亏和权重。
