...
- `run_id` 可选；不传时默认使用最近一次策略运行。
- 报告正文为 Markdown，中文为主，保留必要英文术语。
- 报告内容包含策略运行概览、核心结论、Alpha 排名、目标组合、风控检查、当前持仓、同指数替代观察、定投建议、调仓建议单、回测摘要和系统说明。
- 调仓建议只作为人工确认前的 suggested rebalance orders，不会自动下单或操作资金。
...
- `id`：本次分析保存到 `agent_analysis_log` 后生成的记录 ID；如果未来调用方显式关闭保存，可能为空。
- `created_at`：本次分析记录的保存时间。
- `final_action`：最终建议。
- `final_score`：综合评分。
- `final_summary`：最终结论。
...
- 不传 `symbol` 时返回最近全部 AI 投研记录。
- 传 `symbol` 时只返回该 ETF 的最近记录。
